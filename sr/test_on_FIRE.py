...
# 支持命令行参数指定配置文件路径
parser = argparse.ArgumentParser(description='Test SuperRetina on FIRE dataset')
...
#   - 列表['S', 'A']: 只对指定类别应用掩膜
#   - 字符串'S,A': 自动转换为列表
apply_mask_categories = config['PREDICT'].get('apply_mask_categories', None)

# 如果配置文件中是字符串，转换为列表
if isinstance(apply_mask_categories, str):
    apply_mask_categories = [c.strip() for c in apply_mask_categories.split(',')]

# 规范化：空列表或None都表示不对任何类别应用掩膜
if apply_mask_categories is not None and isinstance(apply_mask_categories, list) and len(apply_mask_categories) == 0:
    apply_mask_categories = None
...
for pair_file in tqdm(match_pairs):
    ...
    # 根据类别决定是否应用掩膜（从配置文件读取）
    # 逻辑说明：
    #   - 如果apply_mask_categories为None或未配置：不对任何类别应用掩膜
    #   - 如果apply_mask_categories为列表：只对列表中的类别应用掩膜
    if apply_mask_categories is not None and len(apply_mask_categories) > 0:
        # 如果配置了类别列表且不为空，根据类别决定是否应用掩膜
        if category in apply_mask_categories:
            # 应用掩膜（如果掩膜存在）
            Pred.set_eye_mask(original_mask)
        else:
            # 不应用掩膜（设置为None）
            Pred.set_eye_mask(None)
    else:
        # 如果未配置类别列表或为空列表，不对任何类别应用掩膜
        Pred.set_eye_mask(None)
...
curve_path = os.path.join(save_dir, "paper_figures", "FIRE_success_rate_vs_threshold_by_category.png")
save_fire_success_curves(auc_record, curve_path, threshold_max=25)
print(f"Success-rate curves (S/P/A) saved to: {curve_path}")

# 保存每对点的详细信息到文本文件
if len(pair_avg_errors) > 0:
    errors_output_path = os.path.join(save_dir, 'FIRE_point_errors.txt')
